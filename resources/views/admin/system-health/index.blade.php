<x-layouts.app
    :title="'System Health'"
    :breadcrumbs="[['label' => 'Administration'], ['label' => 'System Health']]">

    <div class="space-y-6">
        {{-- Summary Row --}}
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="card flex items-center gap-4">
                <div class="flex h-11 w-11 items-center justify-center rounded-xl {{ $score >= 80 ? 'bg-emerald-50 text-emerald-600' : ($score >= 50 ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600') }}">
                ...
                <div>
                    <p class="text-2xl font-bold text-ink-900">{{ $score }}<span class="text-sm font-semibold text-ink-400">%</span></p>
            ...
            <div class="card flex items-center gap-4">
                ...
                <div>
                    <p class="text-2xl font-bold text-emerald-600">{{ $passed }}</p>
            ...
            <div class="card flex items-center gap-4">
                ...
                <div>
                    <p class="text-2xl font-bold text-amber-600">{{ $warned }}</p>
                    <p class="text-xs text-ink-500">Warnings</p>
                </div>
            </div>

            <div class="card flex items-center gap-4">
                <div class="flex h-11 w-11 items-center justify-center rounded-xl {{ $failed > 0 ? 'bg-red-50 text-red-600' : 'bg-ink-100 text-ink-500' }}">
                ...
                <div>
                    <p class="text-2xl font-bold {{ $failed > 0 ? 'text-red-600' : 'text-ink-900' }}">{{ $failed }}</p>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="card px-4 py-3">
                ...
                <p class="mt-1 text-sm font-semibold text-ink-800">{{ $phpVersion }}</p>
            </div>
            <div class="card px-4 py-3">
                <p class="text-[10px] font-bold uppercase tracking-widest text-ink-400">Laravel</p>
                <p class="mt-1 text-sm font-semibold text-ink-800">{{ $laravelVersion }}</p>
            </div>
            <div class="card px-4 py-3">
                <p class="text-[10px] font-bold uppercase tracking-widest text-ink-400">Server Time</p>
                <p class="mt-1 text-sm font-semibold text-ink-800">{{ $serverTime }} {{ $timezone }}</p>
            </div>
            <div class="card px-4 py-3">
                <p class="text-[10px] font-bold uppercase tracking-widest text-ink-400">Uptime</p>
                <p class="mt-1 text-sm font-semibold text-ink-800">{{ $uptime }}</p>
            </div>
        </div>
                    ...
                    <div class="mb-3 flex items-start justify-between gap-2">
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                ...
                                @endif
                                <h3 class="truncate text-sm font-semibold text-ink-800 dark:text-ink-100">{{ $check['name'] }}</h3>
                            </div>
                            <p class="mt-1 text-xs text-ink-400 line-clamp-2">{{ $check['description'] }}</p>
                        </div>
                        <span class="badge shrink-0
                            {{ $check['status'] === 'pass' ? 'badge-green' : ($check['status'] === 'warning' ? 'badge-amber' : 'badge-red') }}">
                            {{ $check['status'] === 'pass' ? 'Pass' : ($check['status'] === 'warning' ? 'Warning' : 'Fail') }}
                        </span>
                    </div>

                    <div class="rounded-lg bg-ink-50 px-3 py-2 dark:bg-ink-800">
                        <p class="text-xs font-medium text-ink-600 dark:text-ink-300">{{ $check['value'] }}</p>
                        @if ($check['detail'])
                            <p class="mt-0.5 text-[11px] text-ink-400 truncate">{{ $check['detail'] }}</p>
                        @endif
                    </div>
                </div>
            @endforeach
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @foreach ($categories as $cat)
                    ...
                    <span class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium
                        {{ $catFails > 0 ? 'bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20' : 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' }}">
                        <span class="h-1.5 w-1.5 rounded-full {{ $catFails > 0 ? 'bg-red-500' : 'bg-emerald-500' }}"></span>
                        {{ ucfirst($cat) }} ({{ $catPassed }}/{{ $catTotal }})
                    </span>
                @endforeach
            </div>
        </div>
