<x-layouts.app :title="'New Import'" :breadcrumbs="[['label' => 'Administration', 'url' => route('admin.imports.index')], ['label' => 'New Import']]">
    ...
    <div class="mx-auto max-w-4xl space-y-6">
        ...
        <div class="rounded-xl border border-ink-200 bg-white p-6">
            <form action="{{ route('admin.imports.upload') }}" method="POST" enctype="multipart/form-data" id="upload-form">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="template_id" class="input">
                        ...
                        @foreach($templates as $template)
                            <option value="{{ $template->id }}">{{ $template->name }} &mdash; {{ ucfirst($template->destination) }}</option>
                        @endforeach
                    </select>
                </div>
