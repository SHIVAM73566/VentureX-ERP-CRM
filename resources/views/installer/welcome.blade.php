                    ...
                    @foreach ($requirements as $req)
                        <div class="flex items-center gap-3 px-6 py-3">
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-medium text-ink-800 dark:text-ink-200">{{ $req['label'] }}</p>
                                <p class="text-xs text-ink-400 dark:text-ink-500">{{ $req['detail'] }}</p>
                            </div>
                            @if ($req['passed'])
                ...
                <div class="px-6 py-4 border-t border-ink-100 dark:border-ink-800 bg-ink-50/50 dark:bg-ink-800/30">
                    @if ($failed === 0)
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('installer.database') }}" class="btn-accent">Continue â†’</a>
                        </div>
                    @else
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 text-sm text-red-700 dark:text-red-400">
                                ...
                                {{ $failed }} requirement(s) failed. Please fix them before continuing.
                            </div>
