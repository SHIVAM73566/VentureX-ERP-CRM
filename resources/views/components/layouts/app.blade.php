...
@php
    ...
    $nav = [
        ...
        'Administration' => [
            ...
            ['label' => 'Export Center', 'icon' => 'file', 'route' => 'admin.exports.index', 'active' => request()->routeIs('admin.exports.*'), 'visible' => $user?->hasAnyPermission(['exports.view', 'exports.export']) || $user?->hasRole('super_admin')],
            ['label' => 'Import Data', 'icon' => 'upload', 'route' => 'admin.imports.index', 'active' => request()->routeIs('admin.imports.*'), 'visible' => $user?->hasAnyPermission(['exports.import']) || $user?->hasRole('super_admin')],
...
<body class="h-full">
    <div x-data="sidebar" x-init="$store.theme.apply()" class="min-h-full">
        ...
        <aside id="sidebar" x-ref="drawer" :class="[open ? 'translate-x-0' : '-translate-x-full', collapsed ? 'lg:w-16' : 'lg:w-64']" class="fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-navy-900 text-slate-300 transition-all duration-200 lg:translate-x-0" aria-label="Sidebar navigation">
            ...
            <nav x-ref="nav" class="flex-1 space-y-5 overflow-y-auto overscroll-contain px-3 py-4">
                @foreach ($nav as $group => $items)
                    ...
                    @endphp
                    <div>
                        <p :class="collapsed ? 'lg:hidden' : ''" class="px-3 pb-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-500">{{ $group }}</p>
                        <ul class="space-y-0.5">
                            @foreach ($items as $item)
                                @php if (! ($item['visible'] ?? true)) { continue; } @endphp
                                <li>
                                    @if (($item['soon'] ?? false) || ! $item['route'])
                                        <span class="group flex cursor-not-allowed items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] text-slate-500" title="Planned" aria-disabled="true">
                                            <svg class="h-4.5 w-4.5 shrink-0 opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons[$item['icon']] ?? $icons['file'] }}"/></svg>
                                    ...
                                    @else
                                        <a href="{{ route($item['route']) }}" @if ($item['active'] ?? false) aria-current="page" @endif class="group flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] {{ ($item['active'] ?? false) ? 'bg-accent-600/90 font-semibold text-white shadow-sm' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}" title="{{ $item['label'] }}">
                                            <svg class="h-4.5 w-4.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons[$item['icon']] ?? $icons['file'] }}"/></svg>
                                            <span :class="collapsed ? 'lg:hidden' : ''" class="truncate">{{ $item['label'] }}</span>
                                        </a>
                                    @endif
                                </li>
            ...
            <div class="border-t border-white/10 p-3">
                <div class="flex items-center gap-2.5 rounded-lg px-2 py-2">
                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-navy-700 text-xs font-bold text-white">{{ $user?->initials() ?? '?' }}</div>
                    <div :class="collapsed ? 'lg:hidden' : ''" class="min-w-0">
                        <p class="truncate text-[13px] font-semibold text-white">{{ $user?->displayName() }}</p>
                        <p class="truncate text-[11px] text-slate-400">{{ $user?->roles->first()?->name }}</p>
                    </div>
                </div>
                <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" title="Sign out" class="mt-2 flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] text-slate-400 hover:bg-white/5 hover:text-white">
                    <svg class="h-4.5 w-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons['logout'] }}"/></svg>
        ...
        <div :class="collapsed ? 'lg:pl-16' : 'lg:pl-64'" class="flex min-h-screen w-full min-w-0 flex-col transition-all duration-200">
            <header class="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-ink-200 bg-white/95 px-4 backdrop-blur lg:px-6 dark:border-ink-800 dark:bg-ink-900/95">
                <button @click="mobileToggle()" x-ref="menuButton" class="rounded-lg p-2 text-ink-500 hover:bg-ink-100 lg:hidden dark:text-ink-400 dark:hover:bg-ink-800" aria-label="Open navigation menu" :aria-expanded="open" aria-controls="sidebar">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="{{ $icons['menu'] }}"/></svg>
                </button>
                ...
                <div class="ml-auto flex items-center gap-1">
                    <button @click="$store.theme.toggle()" class="rounded-lg p-2 text-ink-500 hover:bg-ink-100 dark:text-ink-400 dark:hover:bg-ink-800" title="Toggle theme" aria-label="Toggle theme">
                        <svg x-show="!$store.theme.dark" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons['moon'] }}"/></svg>
                        <svg x-show="$store.theme.dark" x-cloak class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons['sun'] }}"/></svg>
                    </button>
                    ...
                    <div x-data="dropdown" class="relative">
                        <button @click="toggle()" @click.outside="close()" class="relative rounded-lg p-2 text-ink-500 hover:bg-ink-100 dark:text-ink-400 dark:hover:bg-ink-800" aria-label="Notifications" aria-haspopup="menu" :aria-expanded="open">
                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons['bell'] }}"/></svg>
                    ...
                    <div x-data="dropdown" class="relative">
                        <button @click="toggle()" @click.outside="close()" class="flex items-center gap-2 rounded-lg p-1.5 hover:bg-ink-100 dark:hover:bg-ink-800" aria-label="Account menu" aria-haspopup="menu" :aria-expanded="open">
                            <span class="flex h-8 w-8 items-center justify-center rounded-full bg-navy-800 text-xs font-bold text-white">{{ $user?->initials() ?? '?' }}</span>
                            <span class="hidden text-sm font-medium text-ink-700 dark:text-ink-200 md:block">{{ $user?->displayName() }}</span>
                            <svg class="h-4 w-4 text-ink-400 dark:text-ink-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons['chevron'] }}"/></svg>
                        </button>
                        <div x-show="open" x-cloak x-transition @click.outside="close()" class="absolute right-0 mt-2 w-56 rounded-xl border border-ink-200 bg-white p-1.5 shadow-lg dark:border-ink-700 dark:bg-ink-800">
                            @if ($user?->hasRole('super_admin') || $user?->can('configure'))
                                <a href="{{ route('admin.settings.index') }}" class="block rounded-lg px-3 py-2 text-sm text-ink-700 hover:bg-ink-50 dark:text-ink-200 dark:hover:bg-ink-700">Settings</a>
                            @endif
                            <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="block rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10">Sign out</a>
                        </div>
                    </div>
                </div>
            </header>
            ...
            @if ($breadcrumbs)
                <nav class="flex items-center gap-2 px-4 pt-4 text-xs text-ink-400 dark:text-ink-500 lg:px-6">
                    <a href="{{ route('dashboard') }}" class="hover:text-ink-600 dark:hover:text-ink-300">Home</a>
                    @foreach ($breadcrumbs as $crumb)
                        <svg class="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icons['chevron'] }}"/></svg>
                        @if (isset($crumb['url']))
                            <a href="{{ $crumb['url'] }}" class="hover:text-ink-600 dark:hover:text-ink-300">{{ $crumb['label'] }}</a>
                        @else
                            <span class="text-ink-600 dark:text-ink-300">{{ $crumb['label'] }}</span>
                        @endif
                    @endforeach
                </nav>
            @endif
            ...
            <main class="min-w-0 flex-1 px-4 py-6 lg:px-6">
                @if ($title)
                    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                        <h1 class="text-xl font-bold text-ink-900 dark:text-ink-50">{{ $title }}</h1>
            ...
            <footer class="border-t border-ink-200 px-6 py-4 text-xs text-ink-400 flex items-center justify-between dark:border-ink-800 dark:text-ink-500">
                <span>© {{ date('Y') }} {{ config('app.name') }} — Universal CRM + ERP + AI Business Operating System</span>
