<x-layouts.app title="Delete My Account" :breadcrumbs="[['label' => 'Settings', 'url' => route('dashboard')], ['label' => 'Delete Account']]">
    ...
    <div class="mx-auto max-w-2xl space-y-6">
        ...
        <div class="card">
            ...
            <div class="grid grid-cols-3 gap-4 text-center">
                <div class="rounded-lg bg-ink-50 p-4">
                    <div class="text-2xl font-bold text-ink-900">{{ number_format($dataSummary['audit_logs']) }}</div>
                    <div class="text-xs text-ink-500">Audit Log Entries</div>
                </div>
                <div class="rounded-lg bg-ink-50 p-4">
                    <div class="text-2xl font-bold text-ink-900">{{ number_format($dataSummary['login_attempts']) }}</div>
                    <div class="text-xs text-ink-500">Login Attempts</div>
                </div>
                <div class="rounded-lg bg-ink-50 p-4">
                    <div class="text-2xl font-bold text-ink-900">{{ number_format($dataSummary['security_events']) }}</div>
        ...
        <div class="card border-red-200 dark:border-red-800">
            ...
            <form method="POST" action="{{ route('privacy.delete') }}" onsubmit="return confirm('Are you absolutely sure? This cannot be undone.')">
                @csrf
                @method('DELETE')
        ...
        <div class="text-center">
            <a href="{{ route('dashboard') }}" class="text-sm text-ink-500 hover:text-ink-700">
                ← Back to Dashboard
            </a>
        </div>
    </div>
