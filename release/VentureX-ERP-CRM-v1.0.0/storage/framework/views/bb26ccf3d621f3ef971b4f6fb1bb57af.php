...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['label', 'value', 'icon' => 'trending', 'color' => 'blue', 'url' => null, 'sub' => null]));
...
foreach (array_filter((['label', 'value', 'icon' => 'trending', 'color' => 'blue', 'url' => null, 'sub' => null]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<div class="stat-card">
    <div class="flex items-start justify-between">
        <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-ink-400 dark:text-ink-500"><?php echo e($label); ?></p>
            <p class="mt-1.5 text-2xl font-bold text-ink-900 dark:text-ink-50"><?php echo e($value); ?></p>
            <?php if($sub): ?>
                <p class="mt-0.5 text-xs text-ink-400 dark:text-ink-500"><?php echo e($sub); ?></p>
            <?php endif; ?>
        </div>
        <span class="stat-icon <?php echo e($palette['bg']); ?> <?php echo e($palette['text']); ?>">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="<?php echo e($icons[$icon] ?? $icons['trending']); ?>"/></svg>
        </span>
    </div>
    <?php if($url): ?>
        <a href="<?php echo e($url); ?>" class="mt-3 inline-flex items-center gap-1 text-xs font-medium text-navy-600 hover:text-navy-500 dark:text-navy-400 dark:hover:text-navy-300">
            View all
...
<?php /**PATH C:\MY_ERP\resources\views/components/dashboard/stat-card.blade.php ENDPATH**/ ?>